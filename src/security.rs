        ...
        if let Some(allowed) = &self.allowed_paths {
            ...
            let target = if path_ref.exists() {
                 ...
                 if let Some(parent) = path_ref.parent() {
                     if parent.exists() {
                         parent.canonicalize().map_err(MlPrepError::IoError)?.join(path_ref.file_name().unwrap())
                     } else {
                     ...
                     return Err(MlPrepError::Unknown(anyhow::anyhow!("Cannot resolve path parent: {:?}", path_ref)));
                 }
            };
            
            let is_allowed = allowed.iter().any(|allowed_base| {
                target.starts_with(allowed_base)
            });

            if !is_allowed {
                return Err(MlPrepError::Unknown(anyhow::anyhow!("Access denied: Path {:?} is not in allowed paths {:?}", target, allowed)));
            }
        }
        Ok(())
    }

...
"#,
    ...
    assert!(!output.status.success(), "Should fail when accessing outside file");
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("PermissionDenied"), "Should report PermissionDenied");
}
...
"#,
    ...
    assert!(output.status.success(), "Should succeed when accessing allowed file");
}
...
fn test_security_log_masking() {
    ...
    fs::write(&input_path, "email,id\nsecret@example.com,1\nsecret@example.com,2").unwrap();
...
"#,
    ...
    assert!(!stderr_enum.contains("secret@example.com"), "Sensitive data value should not be in logs");

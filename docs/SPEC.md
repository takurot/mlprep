...
### 3.9.4 高度なパフォーマンス最適化（Phase 3）

以下の最適化手法により、さらなる高速化を目指す。

#### 3.9.4.1 GPU アクセラレーション

* **polars-gpu / cuDF 連携**:
  * NVIDIA CUDA GPUを活用した大規模データ処理
  * 対象操作: GroupBy、Join、Window関数、数値フィルタリング
  * 期待効果: 10〜100倍の高速化（データサイズ・操作による）
* **条件付き有効化**:
  * `--gpu` フラグまたは `runtime.gpu: true` で有効化
  * GPU未検出時は自動的にCPUフォールバック

#### 3.9.4.2 SIMD/ベクトル化強化
...
* **対象処理**:
  ...
  * MinMax/Standard scaling の batch 処理
  * OneHot/Count encoding のルックアップ
...
* **効果**:
  * コールドスタート時間の短縮
  * メモリ使用量の削減（特に部分的な列アクセス時）

#### 3.9.4.5 クエリプランキャッシング

* **パイプライン最適化結果のキャッシュ**:
  * 同一YAML設定の再実行時、最適化済みプランを再利用
  * キャッシュキー: パイプラインハッシュ + 入力スキーマハッシュ
* **保存場所**: `.mlprep_cache/plans/`

#### 3.9.4.6 チャンクレベル並列特徴量処理

* **fit/transform の並列化**:
  * DataFrameを論理チャンクに分割し並列処理
  * 統計量計算（mean, std）のマージ処理
* **スレッドプール活用**:
  * Rayonによる自動負荷分散
  * CPU コア数に応じたスケーリング
